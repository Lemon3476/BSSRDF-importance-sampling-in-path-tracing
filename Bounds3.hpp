...
class Bounds3
{
  public:
    ...
    inline bool IntersectBound(Ray& ray, const Vector3f& invDir,
                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    inline bool Bounds3::IntersectBound(Ray& ray, const Vector3f& invDir,
                                const std::array<int, 3>& dirIsNeg)
{
	const auto& origin = ray.origin;
	float tEnter = -std::numeric_limits<float>::infinity();
    ...
    ray.t_min = std::min(tEnter, tExit);
    ray.t_max = std::max(tEnter, tExit);
    return tEnter <= tExit&& tExit >= 0;
}
